...
function Initializer() {
    ...
    if (!isMobile) {
        ...
        var cook = "data-toggle=offcanvas",
            cookie = Cookies.get(cook);

        if (cookie == 1) {
            $("[" + cook + "]").removeClass("collapsed");
            $('.row-offcanvas').addClass("active no-animate");
            $('body').addClass("no-animate");
        }
        setTimeout(function () {
            $('.row-offcanvas, body').removeClass("no-animate");
        }, 300);
    }
    ...
    setTimeout(function () {
        $('.row-offcanvas').removeClass("no-animate");
    }, 300);
    ...
    $('[data-tagging][data-create=false]').selectize({
    ...
    $('[data-tagging][data-create=true]').selectize({
        plugins: ['remove_button'],
        valueField: 'name',
        labelField: 'name',
        searchField: 'name',
...
$(document).on('click', '[data-toggle=offcanvas]', function () {
    var t = $(this),
        cook = "data-toggle=offcanvas";

    if (!t.hasClass('active')) { // kapalı iken aç

        $('.row-offcanvas').addClass('active');
        $('[data-toggle=offcanvas]').addClass('active');
...
$(document).on('click', '[data-switchInput]', function (e) {
    var t = $(this),
        w = t.parents('[data-switchWrapper]'),
        f = w.find('.form-control');
